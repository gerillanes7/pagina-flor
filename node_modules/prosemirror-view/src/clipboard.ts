...
function maybeWrapTrusted(html: string): string {
  ...
  if (!_policy) {
    if (_policy = trustedTypes.defaultPolicy)
      try { return _policy.createHTML(html) } catch {}
    _policy = trustedTypes.createPolicy("ProseMirrorClipboard", {createHTML: (s: string) => s})
  }
    ...
    for (let j = 0; j < style.rules.length; j++) {
      ...
      if (rule instanceof CSSStyleRule) {
        ...
        for (let k = 0; k < matches.length; k++)
          (matches[k] as HTMLElement).style.cssText += rule.style.cssText
      }
    }
  }
  return elt
