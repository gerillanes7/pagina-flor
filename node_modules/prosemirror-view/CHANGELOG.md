...
Fix an issue where some kinds of deletions on Chrome Android were interpreted as Enter presses.

Fix a regression where some types of Enter presses on Android would not go through the Enter key handling.

## 1.42.4 (2026-09-18)

### Bug fixes

Prevent uneditable elements rendered as part of node or mark view from blocking cursor motion.

Fix a bug that could break some types of composition input starting with a non-empty selection.

## 1.42.3 (2026-08-25)

### Bug fixes

Don't set `contenteditable=false` on widgets that already have a `contenteditable` attribute.

Fix a bug that broke pasting when there was a default trusted type policy defined without a `createHTML` function.

Run attribute validators on attributes provided via slice context in clipboard content. This closes an XSS vulnerability (found by Pedro Paniago).

## 1.42.2 (2026-07-24)

### Bug fixes

Enable a workaround for backspace not taking effect on Android for Firefox as well as Chrome (for which it already kicked in).

Work around the situation where Chrome/Webkit replace a node's content DOM with a completely different element.

## 1.42.1 (2026-07-11)

### Bug fixes

Don't scroll to a bogus position when trying to scroll an element that has no layout into view.
